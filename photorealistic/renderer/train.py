...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for idx, data in enumerate(dataset):
        ...
        n_frames_load = min(n_frames_load, n_frames_total - opt.n_frames_G + 1)
        t_len = n_frames_load + opt.n_frames_G - 1
        fake_B_last = None
        ...
        for i in range(0, n_frames_total-t_len+1, n_frames_load):
            ...
            mouth_centers = Variable(data["mouth_centers"][:, i:i+t_len, ...]).view(-1, t_len, 2)
            eyes_centers = None
            ...
            if mouth_centers is not None:
                mouth_centers = mouth_centers[:, (opt.n_frames_G - 1):, :].contiguous().view(-1, 2)
            if eyes_centers is not None:
                eyes_centers = eyes_centers[:, (opt.n_frames_G - 1):, :].contiguous().view(-1, 2)
