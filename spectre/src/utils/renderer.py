...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256):
        ...
        self.verts, faces, aux = load_obj(obj_filename)
    ...
    def render_shape(self, vertices, transformed_vertices, images=None, detail_normal_images=None, lights=None):
        ...
        # Attributes
        batch_faces = self.faces.expand(batch_size, -1, -1)
        face_vertices = util.face_vertices(vertices, batch_faces)
        normals = util.vertex_normals(vertices, batch_faces)
        face_normals = util.face_vertices(normals, batch_faces)
        transformed_normals = util.vertex_normals(transformed_vertices, batch_faces)
        transformed_face_normals = util.face_vertices(transformed_normals, batch_faces)
        attributes = torch.cat(
            [
        ...
        # Rasterize
        rendering = self.rasterizer(transformed_vertices, batch_faces, attributes)
    ...
    def render_colors(self, transformed_vertices, colors, pos_mask=None):
        ...
        batch_faces = self.faces.expand(batch_size, -1, -1)
        attributes = util.face_vertices(colors, batch_faces)
        # Rasterize
        rendering = self.rasterizer(transformed_vertices, batch_faces, attributes)
